...
public class ChatController {
    ...
    public ResponseEntity<RequestOutput> chat(
        ...
        FinalResponse orchestratorResponse = chatService.processUserQuery(input.prompt());
        log.info("Sending chat response: {}", orchestratorResponse);

        ResponseCookie cookie = ResponseCookie.from("X-CONV-ID", conversationId)
                .path("/")
                .maxAge(3600)
                .build();
        RequestOutput output = new RequestOutput(orchestratorResponse.analysis(), orchestratorResponse.workerResponses());
    ...
    public record RequestOutput(String content, List<String> workerResponses) {
    }
}
