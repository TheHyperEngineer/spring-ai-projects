...
public class ChatController {
    ...
    public ResponseEntity<RequestOutput> chat(
        ...
        List<String> responses = chatService.processUserQuery(input.prompt());
        log.info("Sending chat response: {}", responses);

        ResponseCookie cookie = ResponseCookie.from("X-CONV-ID", conversationId)
                .path("/")
                .maxAge(3600)
                .build();
        RequestOutput output = new RequestOutput(responses);
    ...
    public record RequestOutput(List<String> content) {
    }
}
