    ...
    public ChatClientResponse adviseCall(ChatClientRequest chatClientRequest, CallAdvisorChain callAdvisorChain) {
        ...
        for (int attempt = 1; attempt <= maxRepeatAttempts + 1; attempt++) {
            ...
            if (this.skipEvaluationPredicate.test(chatClientRequest, response)) {
                log.debug("Skipping evaluation because skipEvaluationPredicate returned true.");
                return response;
            }
            ...
            if (evaluation.rating() >= this.successRating) {
                log.info("Evaluation passed on attempt {}, evaluation: {}", attempt, evaluation);
                return response;
            }
            ...
            log.warn("Evaluation failed on attempt {}, evaluation: {}, feedback: {}", attempt,
